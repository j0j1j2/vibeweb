...
**Flow (per-tenant):**
1. Tenant owner clicks "Connect Claude" on their Settings page
2. Console calls `POST /auth/claude/:tenantId/login` on Agent Service
3. Agent Service spawns a temporary session container running `claude auth login`
   - Container bind-mounts `/data/tenants/{tenantId}/claude-auth/` as `/root/.claude/`
4. The CLI outputs an OAuth URL — Agent Service captures it and returns to user
5. User opens the URL in their browser, authenticates with Anthropic
6. The callback page shows an authorization code
7. User pastes the code into the console UI
8. Console calls `POST /auth/claude/:tenantId/code` with the code
9. Agent Service pipes the code to the waiting container's stdin
10. Claude CLI completes authentication and saves credentials
11. Session containers for that tenant bind-mount the same credential directory

**Agent Service endpoints:**

| Method | Path | Description |
|--------|------|-------------|
| `POST` | `/auth/claude/:tenantId/login` | Start `claude auth login` in temp container, return OAuth URL |
| `POST` | `/auth/claude/:tenantId/code` | Submit the authorization code to complete login |
| `GET` | `/auth/claude/:tenantId/status` | Check if tenant has credentials |
| `DELETE` | `/auth/claude/:tenantId` | Remove tenant credentials |

**Credential storage:**
- Stored at `/data/tenants/{tenantId}/claude-auth/` (per-tenant directory)
- Bind-mounted into session containers as `/root/.claude/` (read-only)
- Each tenant manages their own Claude account
- Fallback: if `ANTHROPIC_API_KEY` env var is set on Agent Service, it's injected as before
