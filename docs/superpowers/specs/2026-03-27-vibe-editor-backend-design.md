...
**Flow:**
1. Admin clicks "Connect Claude" on the Admin Dashboard page
...
3. Agent Service spawns a temporary session container running `claude login`
   - The temp container shares the `claude-auth` named volume at `/root/.claude/`
...
6. Claude CLI saves credentials to the `claude-auth` volume
7. Agent Service polls for credential file, returns success when found
8. All subsequent session containers mount the same `claude-auth` volume at `/root/.claude/`
...
**Credential storage:**
- Stored in Docker named volume `claude-auth` (shared between agent-service + session containers)
- Mounted into agent-service at `/data/claude-auth/` (for status checks)
- Mounted into session containers at `/root/.claude/` (for CLI authentication)
- Platform-wide (not per-tenant) — admin manages one Claude account for all sessions
- Only accessible from the Admin Dashboard (not tenant Settings)
